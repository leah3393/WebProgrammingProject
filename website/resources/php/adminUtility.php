<?php
	echo 
		"<html> 
			<head> 
				...
				<link href=\"adminUtility.css\" type=\"text/css\" rel=\"stylesheet\" />
			</head>
			<body>
	...
	$sql = "SELECT sid, fname, lname, email, phone, addr, agency FROM seller WHERE approved = 'false'";
	...
	while($row = mysqli_fetch_array($result))
	{
		...
		echo "<p id=\"paragraph" . $counter . "\">" . $row['sid'] . " " . $row['fname'] .  " " . $row['lname'] . " from " . $row['agency'] . " </p>";
	...
	echo
			"<script src=\"adminUtility.js\"></script>";
	echo
			"</body>
		</html>";
	mysqli_close($con);
